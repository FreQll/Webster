...
const UserProfile = () => {
  ...
  const handleDelete = async (canvasId: string) => {
    if (selectedCanvas.id == canvasId) {
      const firstCanvas = await axios.get(`/canvas/user/${user.id}`, GET_CONFIG);
    ...
    <div className='px-12 flex flex-wrap gap-5'>
      {allProjects.length > 0 && (
        allProjects.map((canvas: Canvas, index) => (
          <div key={index} className='group flex flex-col gap-2 relative cursor-pointer' onDoubleClick={() => openCanvas(canvas.canvasJSON, canvas.id)}>
            <div className='w-[200px] md:w-[400px] h-[100px] md:h-[250px] flex items-center justify-center rounded-[13px] border-[2px]'>
              <img src={canvas.imageURL} alt="" className='max-w-[100%] max-h-[100%]' />
              <div className={`absolute top-2 right-2 border-[2px] rounded-[8px]`}>
                <DropdownMenu>
                  <DropdownMenuTrigger className='pb-2 px-2 outline-none'>...</DropdownMenuTrigger>
                  <DropdownMenuContent>
                    <DropdownMenuLabel className='cursor-pointer'>
                      <Dialog>
                        <DialogTrigger className='w-[100%] text-left outline-none'>Edit</DialogTrigger>
                        ...
                        <DialogContent className="sm:max-w-[425px]">
                          ...
                          <DialogFooter>
                            <Button onClick={() => handleUpdate(canvas)}>Save changes</Button>
                          </DialogFooter>
                        </DialogContent>
                      </Dialog>
                    </DropdownMenuLabel>
                    ...
                    <DropdownMenuLabel className='cursor-pointer text-red-500' onClick={() => handleDelete(canvas.id)}>Delete</DropdownMenuLabel>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
            </div>
            <div className='ml-2' onClick={() => openCanvas(canvas.canvasJSON, canvas.id)}>
              <div className='font-[500] cursor-pointer'>{canvas.name}</div>
              <div className='text-gray-700 text-[14px]'>{`Edit ${getLastEdit(canvas.updatedAt)} ago`}</div>
            </div>
          </div>
        ))
      )}
